...
```
After fitting the logistic model and computing backwards elimination, we found the best model for our response variable(Factors.Threat.Level) to be an additive model between our four explanatory varibales: gender, race, armed status, and age. 
Therefore, whether or not a victim was seen as a threat was explaned with an additive interaction between gender, race, armed status, and age. 
